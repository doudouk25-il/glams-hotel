...
# Cherche s'il existe deja une sauvegarde complete
$fullBackup = Get-ChildItem $backupDir -Filter "glamshotel_FULL_*.zip" |
              Sort-Object LastWriteTime -Descending |
              Select-Object -First 1

if (-not $fullBackup) {

    # --- SAUVEGARDE COMPLETE (premiere fois uniquement) ---
    ...
    # --- SAUVEGARDE INCREMENTIELLE (modifications seulement) ---
    $lastAny = Get-ChildItem $backupDir -Filter "glamshotel_*.zip" |
               Sort-Object LastWriteTime -Descending |
               Select-Object -First 1
    $since = $lastAny.LastWriteTime
    ...
    if ($changedFiles.Count -eq 0) {
        Write-Host "Aucun fichier modifie depuis la derniere sauvegarde." -ForegroundColor Yellow
        exit 0
    }

    $zipName = "glamshotel_INCR_" + $timestamp + ".zip"
    $zipPath = Join-Path $backupDir $zipName
    $sinceStr = $since.ToString("dd/MM/yyyy HH:mm")
    Write-Host ($changedFiles.Count.ToString() + " fichier(s) modifie(s) depuis " + $sinceStr + "...") -ForegroundColor Cyan
